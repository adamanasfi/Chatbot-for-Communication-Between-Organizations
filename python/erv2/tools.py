...
class ERA2ATools:
    def __init__(self, graph: Optional[Any], interagent_thread_id: Optional[str] = None):
        ...
        self.interagent_thread_id = interagent_thread_id or os.getenv(
            "INTERAGENT_THREAD_ID", "er_hcc_coord_case_1"
        )
        self.hcc_peer = A2APeer(
            base_url=os.getenv("HCC_BASE_URL", "http://127.0.0.1:2027")
        )
        self.send_to_hcc_a2a_tool = tool(self.send_to_hcc_a2a)
    ...
    async def _append_to_interagent_memory(
        ...
        # ER-local memory projection:
        # ER outbound -> AIMessage, HCC reply -> HumanMessage.
        values = {
            "messages": [
                AIMessage(
                    content=(
    ...
    async def send_to_hcc_a2a(self, message_text: str) -> str:
        """
        ER -> HCC (A2A)
        """
        tagged_text = (
        ...
        try:
            response_text, _, _ = await self.hcc_peer.send_text(
                text=tagged_text,
            ...
            response_text = (
                f"[A2A_ERROR] Could not contact HCC: {type(e).__name__}: {e}"
            )
