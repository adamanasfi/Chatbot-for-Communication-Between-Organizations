...
class HospitalCoordinatorAgent:
    COORDINATOR_SYSTEM_PROMPT = """
    You are the Hospital Coordinator Agent.

    You only communicate with the Red Cross Coordinator Agent over the inter-organization channel.
    You do not communicate with hospital employees directly.
    ...
    - You may call send_to_red_cross_a2a when Red Cross coordination is required.
    ...
    - If the incoming message contains:
      ...
      send_to_red_cross_a2a.
    - Use send_to_red_cross_a2a only when initiating a NEW outbound request.
    """.strip()
    ...
    def __init__(self):
        ...
        self.handoff_thread_id = os.getenv(
            "HOSPITAL_HANDOFF_THREAD_ID", "hospital_llm_to_hospital_coord_case_1"
        )
        self.intercoord_thread_id = os.getenv(
            "INTERCOORD_THREAD_ID", "hospital_redcross_coord_case_1"
        )
        self.tools_service = HospitalA2ATools(
