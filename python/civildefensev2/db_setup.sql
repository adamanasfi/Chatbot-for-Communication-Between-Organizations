...
-- Latest known position of vehicles tracked by partner organizations
-- (currently Red Cross), written directly by that partner's process --
-- no A2A call, no LLM inference on either side. Read on demand by a
-- Civil Defense agent tool; nothing here triggers that automatically.
