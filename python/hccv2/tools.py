...
class HCCA2ATools:
    def __init__(self, graph: Optional[Any], interagent_thread_id: Optional[str] = None):
        ...
        self.er_peer = A2APeer(
            base_url=os.getenv("ER_BASE_URL", "http://127.0.0.1:2026")
        )
        self.send_to_er_a2a_tool = tool(self.send_to_er_a2a)
    ...
    async def _append_to_interagent_memory(
        ...
        # HCC-local memory projection:
        # HCC outbound -> AIMessage, ER reply -> HumanMessage.
        values = {
            "messages": [
                AIMessage(
                    content=(
    ...
    async def send_to_er_a2a(self, message_text: str) -> str:
        """
        HCC -> ER (A2A)
        """
        tagged_text = (
        ...
        try:
            response_text, _, _ = await self.er_peer.send_text(
                text=tagged_text,
            ...
            response_text = (
                f"[A2A_ERROR] Could not contact ER: {type(e).__name__}: {e}"
            )
